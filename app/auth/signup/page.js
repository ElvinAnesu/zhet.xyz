...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!success) {
        throw new Error(error || "Failed to create account. Please try again.");
      }
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (!success) {
        throw new Error(error || "Google sign up failed. Please try again.");
      }
      ...
      <div className="mt-6 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white dark:bg-slate-800 py-6 px-4 shadow sm:rounded-lg sm:px-6">
          ...
          <form className="space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="fullName" className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300">
                Full Name
              </label>
              <div className="mt-1">
                <input
